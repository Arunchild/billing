    ...
    <div class="row">
        <div class="col-md-6">
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <div class="mb-3">
                        <label class="form-label">Reasons / Notes</label>
...
<script>
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if(response && response.items) {
                    populateItemsFromInvoice(response.items);
                }
            },
            error: function() {
    ...
    function populateItemsFromInvoice(items) {
        ...
        items.forEach(item => {
            rowCount++;
            // Generate product options restricted to this invoice items ?? 
            // The user said: "selecting invoice number it should have all the items names in dropdown belong to that invoice"
            // So we should ideally replace the global product list in the select dropdown with ONLY these items.
            // But usually returns are partial, so we might want to list all items as separate rows initially?
            // "selecting one and adding then we can finish it" implies we start empty or with full list?
            // Interpretation: The dropdown in the row should ONLY show items from the invoice.
            
            // Let's create a row for EACH item in the invoice, but allow user to remove them or adjust qty.
            // OR: Let's make the dropdown only contain these products.
            
            // Strategy: Pre-fill rows with invoice items for ease.
            
            const newRowHtml = `
            <tr class="item-row animate__animated animate__fadeIn">
                <td>
                    <select name="items[${rowCount}][product_id]" class="form-select product-select select2-new-${rowCount}" onchange="updateProduct(this)" required>
                        <option value="${item.product_id}" data-price="${item.price}" selected>${item.product.name} ${item.product.item_code ? '('+item.product.item_code+')' : ''}</option>
                    </select>
                    <input type="hidden" name="items[${rowCount}][product_name]" class="product-name" value="${item.product_name}">
                </td>
                <td>
                    <input type="number" name="items[${rowCount}][quantity]" class="form-control qty-input text-center" value="${item.quantity}" min="1" max="${item.quantity}" onchange="calculateRow(this)" onkeyup="calculateRow(this)">
                </td>
                <td>
                    <input type="number" name="items[${rowCount}][price]" class="form-control price-input text-end" step="0.01" value="${item.price}" onchange="calculateRow(this)" onkeyup="calculateRow(this)">
                </td>
                <td>
                    <input type="number" name="items[${rowCount}][tax_rate]" class="form-control tax-rate-input text-center" step="0.01" value="${item.tax_rate}" onchange="calculateRow(this)" onkeyup="calculateRow(this)">
                </td>
                <td>
                    <input type="number" name="items[${rowCount}][tax_amount]" class="form-control tax-amount-input text-end bg-light" step="0.01" value="${item.tax_amount}" readonly>
                </td>
                <td>
                    <input type="number" name="items[${rowCount}][total]" class="form-control total-input text-end fw-bold bg-light" step="0.01" value="${item.total}" readonly>
                </td>
                <td class="text-center align-middle">
                    <button type="button" class="btn btn-outline-danger btn-sm rounded-circle" onclick="removeRow(this)"><i class="fas fa-times"></i></button>
                </td>
        ...
        calculateTotals();
        toastr.success('Invoice items loaded. Adjust quantities as needed.');
    }
            ...
            $.ajax({
                ...
                success: function(response) {
                    if(response.success) {
                        ...
                        addRow(); 
                        $('input[name="return_number"]').val(response.next_return_number);

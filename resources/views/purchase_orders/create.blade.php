...
<div class="d-flex justify-content-between align-items-center mb-4 animate__animated animate__fadeIn">
    <div>
        <h2 class="mb-1 fw-bold text-primary">{{ isset($purchaseOrder) ? 'Edit Purchase Order' : 'New Purchase Order' }}</h2>
        <p class="text-muted mb-0">{{ isset($purchaseOrder) ? 'Modify order details' : 'Create a new purchase order' }}</p>
    </div>
    <div>
...
<form action="{{ isset($purchaseOrder) ? route('purchase_orders.update', $purchaseOrder->id) : route('purchase_orders.store') }}" method="POST" id="purchaseOrderForm" class="animate__animated animate__fadeInUp">
    @csrf
        ...
        <div class="card-body">
            <div class="row g-3">
                    ...
                    <div class="input-group">
                        <select name="supplier_id" class="form-select select2" required>
                            ...
                            @foreach($suppliers as $supplier)
                                <option value="{{ $supplier->id }}" {{ (isset($purchaseOrder) && $purchaseOrder->supplier_id == $supplier->id) ? 'selected' : '' }}>{{ $supplier->name }}</option>
                            @endforeach
                        </select>
                ...
                <div class="col-md-2">
                    ...
                    <input type="text" name="po_number" class="form-control" value="{{ isset($purchaseOrder) ? $purchaseOrder->po_number : $poNumber }}" readonly>
                </div>
                <div class="col-md-3">
                    <label class="form-label">Order Date <span class="text-danger">*</span></label>
                    <input type="date" name="po_date" class="form-control" value="{{ isset($purchaseOrder) ? $purchaseOrder->po_date : date('Y-m-d') }}" required>
                </div>
                <div class="col-md-3">
                    <label class="form-label">Delivery Date</label>
                    <input type="date" name="delivery_date" class="form-control" value="{{ isset($purchaseOrder) ? $purchaseOrder->delivery_date : '' }}">
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="card mb-4">
        <div class="card-header d-flex justify-content-between align-items-center">
            <span>Items</span>
            <span class="badge bg-info text-dark">Order Items</span>
        </div>
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <div class="mb-3">
                        ...
                        <textarea name="notes" class="form-control" rows="2" placeholder="Notes...">{{ isset($purchaseOrder) ? $purchaseOrder->notes : '' }}</textarea>
                    </div>
                </div>
            </div>
        </div>
        <div class="col-md-6">
            <div class="card bg-white">
                ...
                <div class="card-footer bg-light text-end">
                    <button type="submit" class="btn btn-success px-4"><i class="fas fa-check"></i> Save Order</button>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    let rowCount = {{ isset($purchaseOrder) ? $purchaseOrder->items->count() : 1 }};
